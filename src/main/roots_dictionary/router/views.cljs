(ns roots-dictionary.router.views
...
(defn view [{:keys router}]
  (let [current-route @(re-posh/subscribe [::current-route])]
    (when (:router/current-route current-route)
      [(-> current-route :router/current-route :data :view)])))
